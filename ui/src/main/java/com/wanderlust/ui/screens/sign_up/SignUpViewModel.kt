...
sealed interface SignUpSideEffect {
    ...
    data class SetUser(val user: User) : SignUpSideEffect
}
...
@HiltViewModel
class SignUpViewModel @Inject constructor(private val register: RegisterUserUseCase) : ViewModel() {
    ...
    private fun onRegisterButtonClick() {
        ...
        currentJob = viewModelScope.launch {
            ...
            when (result) {
                is SuccessRegister -> {
                    _action.emit(SignUpSideEffect.SetUser(result.user))
    ...
    private fun validateFields(errors: MutableList<String>): Boolean {
        ...
        if (!android.util.Patterns.EMAIL_ADDRESS.matcher(state.value.email).matches()) {
            errors.add("Неверный формат пароля.")
            return false
        }
